...
# relaxation zones as described in [1]
#
### Please note ###
# Recommendations are given in [1] how to tune the relaxation zone's parameters depending on the waves. See also the manual distributed along with the code.
# Although theory predictions are often quite accurate [1], please keep in mind that every theory has its limitations! Tuning the relaxation zone parameters according to the present theory does NOT guarantee that the actual reflection coefficient in the simulation will equal the prediction. Other mechanisms can also lead to undesired wave reflections. Thus it is recommended to select zone thickness slightly larger than theoretically necessary.
# The theory predictions were found to be of satisfactory accuracy for regular, irregular, linear, highly nonlinear, 2D-, and 3D-waves [1,2,3]. Indeed, the present program for 1D-wave-propagation (i.e. 2D-flow with wave propagation in one direction) was found to be of satisfactory accuracy for typical 3D-flow simulations [1,2].
...
# [3] Perić, R., & Abdel-Maksoud, M. (2018). Analytical prediction of reflection coefficients for wave absorbing layers in flow simulations of regular free-surface waves. Ocean Engineering, 147, 132-147.
#
...
#
### Bug fixes ###
# The 2017 version of this code was slightly inaccurate for shallow water. This has been amended, and now the code should work for all water depths.
#
...
if (len(sys.argv)-1 == 5):
	...
	h = float(sys.argv[2])
	L = float(sys.argv[3])
	zone_thickness_xd = float(sys.argv[4])
	blend = float(sys.argv[5])
	#pylab_available = "NO"
...
# interactive mode
	...
	zone_thickness_xd = float(input("\nPlease enter relaxation zone thickness (m):\n"))
...
tauMin=10**-8*T*forcedEQs		# range of relaxation parameter tau for which reflection coefficient is computed
tauMax=10**4*T*forcedEQs		# -"-
dampres=200				# number of piece-wise constant blending zones, into which relaxation zone is subdivided
Lx = 2.0*L 				# for plots: domain size outside the relaxation zone
...
Ct = [0.0] * ( int(dampres) + 2 )		 	# initialize vector for transmission coefficient, Ct[1] is coefficient at the entrance to the relaxation zone
beta = [0.0] * ( int(dampres) + 2 )			# initialize vector needed to compute Cr
Cr = [0.0] * ( int(dampres) + 2 )		 	# initialize vector for reflection coefficient, Cr[1] is the global reflection coefficient (i.e. for the entrance to the relaxation zone)

# write initial conditions
dx = zone_thickness_xd / dampres	# thickness of single 'cell' within damping layer
xd[1] = Lx
...
def b(x):
	if (x >= Lx):
		...
		# constant	
		if (blend == 1):
			w = 0.5	# constant blending

		# linear
		elif (blend == 2):
			w = ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx )

		# quadratic
		elif (blend == 3):
			w = ( ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx ) )**2

		# cosine-squared
		elif (blend == 4):
			w = math.cos( 0.5 * math.pi + 0.5 * math.pi * ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx ) )**2

		# exponential
		elif (blend == 5):
			w = (math.exp( ( ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx )  )**2 ) -1) / ( math.exp(1) - 1 )
		# power with exponent n
		elif (blend == 6):
			w = ( ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx ) )**n
		# exponential with exponent n
		elif (blend == 7):
			w = (math.exp( ( ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx )  )**n ) -1) / ( math.exp(1) - 1 )
		# cosine-squared with exponent n
		elif (blend == 8):
			w = ( math.cos( 0.5 * math.pi + 0.5 * math.pi * ( x - Lx ) / ( ( Lx + zone_thickness_xd ) - Lx ) )**2 )**n
...
while (tau < tauMax):
	tmp = Lx
	for i in range(2,dampres+2,1):
		k[i] = cmath.sqrt(  (w**2)/ ( c**2 ) + (1j * w * tau * b( tmp + 0.5 * xd[i] ) ) / ( (c)**2 ))
		tmp += xd[i]

	# calculate transmission and reflection coefficients
	for i in range(dampres,0,-1):   # go cell-by-cell from the end of the relaxation zone to its entrance. E.g. for 3 zones, start with zone 3, then 2, then 1.
		beta[i+1] = fbeta(i+1)
		Cr[i] = fCr(i)
		Ct[i] = fCt(i)
...
if (pylab_available == "YES"):
	...
	pylab.title('Reflection coefficient $C_R$ versus relaxation parameter $\\tau$')
	pylab.semilogx()
	pylab.semilogy()
